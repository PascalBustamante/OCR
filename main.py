...
def get_hog_features(img, orient, pix_per_cell, cell_per_block, vis=True, feature_vec=True):
    ...
    blurred_img = cv2.GaussianBlur(img, (5, 5), 0)  # Adjust kernel size (e.g., (5, 5)) and sigma as needed
...
def hog_segmentation(image, orient, pix_per_cell, cell_per_block, threshold):
    ...
    features, _ = get_hog_features(gray, orient, pix_per_cell, cell_per_block, vis=True, feature_vec=False)
    ...
    reshaped_features = features.reshape((num_cells_y, num_cells_x,  cell_per_block, cell_per_block, orient))
...
# Read in the image
image = mpimg.imread(r"C:\Users\pasca\Data Science\Math Notes Model\STN\test_data\math_example.png")
...
labeled_mask, regions, binary_mask = hog_segmentation(image, orient, pix_per_cell, cell_per_block, threshold)
